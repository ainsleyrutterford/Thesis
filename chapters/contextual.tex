...
Although there exists many examples of coral species classification using deep learning, to the best of the author's knowledge, no machine learning techniques of any kind have ever been applied to CT scans of coral skeletons and, by extension, the implementation of an automated system to extract the density banding or calculate the linear extension and calcification rates has not yet been attempted or at least published.
...
need to talk about unet on density data segmenting. does it in a skull i think ive seen? however no class imbalance problem as areas are quite large or thick. maybe also mention that the original biomedical data is not too disimilar to our data in that it has one channel and the boundaries are thin?
