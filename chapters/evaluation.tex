...
The first ablation study was used to determine the importance of the pass-forward of information from the contracting path to the expanding path that the U-Net architecture is known for. other than the removal of all concatenation operations, the rest of the architecture was unchanged. With this pass-forward of information removed the network performed surprisingly well. Although a decrease in the validation accuracy of ${\sim}4\%$ was observed, the predictions produced were qualitatively better in some respects. When looking at Figure \ref{fig:ablation} for example, it can be seen that the produced boundaries are significantly smoother and that the ``continuity'' of the boundaries is rarely broken. As discussed in Chapter \ref{chap:implementation}, smooth boundaries are deemed as more realistic.
...
The performances achieved by the various ablated networks are summarised in Table \ref{tab:ablation}. Although the ablated network with no pass-forward of information and the network with a reduced number of feature channels both produced acceptable predictions, the unablated network still performed the best and was used for the final results shown in Section \ref{sec:finalresults}.
...
% The manual splits chosen are outlined in Tables \ref{tab:appendixtab}--\ref{tab:appendixtab4}.
...
Although the custom accuracy metric implemented does solve many of the problems faced by a classic per-pixel accuracy metric, it is not without its own shortcomings.

The most obvious shortcoming of the custom accuracy metric is that it has no way of telling whether the pixels it is comparing should belong to the same boundary or not. For example, the prediction of boundaries

The most obvious shortcoming highlighted in Section \ref{sec:evalablation} is the fact that breaks in continuity are not punished enough. Breaks in continuity are indirectly punished to an extent since a break in continuity results in the corresponding area of a ground truth boundary having no pixels close to it. However, the continuity of the boundaries is essential for the estimation of the calcification rate and should thus contribute to the accuracy achieved far more than it does currently.
...
Doesn't punish breaks in continuity.
