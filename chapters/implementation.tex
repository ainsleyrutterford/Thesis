...
In order to train this fully 3D architecture, a new dataset containing patches nine-patch samples and labels was curated using the same sliding window technique. Since only a small selection of slices were labelled initially, a larger selection of slices adjacent to the initially selected slices were labelled manually. Ultimately, labelling enough data for this model to perform well proved challenging---curating a dataset with the same amount of samples used to train the 2D architecture would require nine times the amount of labelled slices.
...
Once the prediction is skeletonized, the ``average'' distance between the prediction and the ground truth label can be calculated. For each white pixel in the skeletonized prediction, the closest white pixel---the pixel with the shortest Euclidean distance---in the label is found. This is then repeated for all pixels in the prediction and the distances are averaged.

For each white pixel in the prediction, find its closest white pixel in the label. Do the same from the label to the prediction. Average all of these distances.

If a completely black image is produced, a single white pixel is placed in the centre of the image to prevent infinity values as these would not contribute to the average whereas very high values do and negatively affect it.

(Show algorithm and maybe some source code?)

\subsection{The \texttt{ctypes} Module}

The distance calculation was $O(n^2)$ or something so took very long time for a single image (give average). In order to calculate the average accuracy achieved on the training set this would take 322 * (time taken) * epochs. Wrapping in ctypes gave a 600 or something times speedup and this allows the average accuracy achieved to take ... seconds per epoch.
