...
In order for an ANN to perform well, large amounts of labelled data much be available to train on. Since the dataset provided was initially unlabelled, a manual labelling process was devised and is described in this section.
...
Not all slices that compose a scan contain annual density banding that can confidently be identified. In order to find appropriate slices, each Porites scan was opened and inspected manually using a 3D visualisation program called Avizo\footnote{\url{https://tiny.cc/avizo}}. Avizo allows users to view slices orthogonal to the $x$, $y$, or $z$ axis of a scan. A selection of seven slices that could confidently be labelled were chosen. Once an appropriate slice was identified, a Python script was used to extract the slice and export it as a greyscale PNG image. Some of the adjacent slices were also extracted in order to curate the 3D dataset used later in the project (discussed in Section~\ref{sec:threedimension}). The Python script used is shown in Listing \ref{lst:orientation}.

\input{listings/orientation}

\subsubsection{Slice Labelling}

Each selected slice was then manually labelled using the GNU Image Manipulation Program (GIMP) [????]. An example of a labelled slice is shown in Fig.~\ref{fig:example-label}. A 1px white line is drawn at the beginning and end of each annual high density band and the rest of the image is black\textemdash no values other than black or white are present in the labels. When labelling the slices, it is not always clear where the annual bands begin and end, as can be seen in Fig.~\ref{fig:example-label} (left). In order to ensure that the decisions made regarding the boundaries of the annual bands were as consistent as possible, each slice was manually labelled multiple times, and the most common boundaries were chosen. It is important for the labelling method to be consistent and reproducible as this enables the dataset to be expanded in the future.
