...
Finally, once the density bands are identified, the estimation of the distance between the bands is automated using various techniques. These distances can then be used to automatically estimate the volume of skeletal matter produced annually.
...
Analysis of the annual banding can also be used to determine changes in a coral sample's environmental conditions as it was growing (import something about global warming maybe that they said in the 2011 paper to show that analysing these density bands is important. look near their conclusion too maybe?)~\cite{lough2011new}. This project aims to automate the estimation of the linear extension rate which is then used to estimate the calcification rate. Deep learning is used to extract the boundaries of each annual density band and the distance between these boundaries is then estimated using various techniques.
...
Automated solution will be deterministic and will give same answer. The bands are not always clear and opinions will differ.

Automated solution once trained will be much faster. Analysing an entire slice will take a matter of seconds

This project makes use of a dataset of more than 160 three dimensional computed tomography (CT) scans of unique coral skeletons from the Natural History Museum's collection. These scans are stored as stacks of \texttt{.tif} images.
